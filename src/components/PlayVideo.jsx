		...
		<div className="w-[100%] lg:w-[69%] ">
			...
			<div >
				...
				<div className={`relative mb-7 h-full`}>
					<p className={`${isOpen ? 'h-full' : (apiData ? (apiData.snippet.description.length > 0 ?'h-[100px]' : 'h-0'):'')} overflow-hidden`}>{apiData ? apiData.snippet.description : ''}</p>
					<button onClick={()=>setIsOpen(!isOpen)} className='absolute bottom-[-1] cursor-pointer font-semibold'>{apiData && !isOpen ? (apiData.snippet.description.length >= 150 ? '...more': ''): '...less'}</button>
				</div>
			</div>
			<div>
				<hr className="h-[1px] border-none bg-[#ccc] m-[10px_0]" />
				...
				{commentData.map((item,index)=>(
					...
					<img src={item.snippet.topLevelComment.snippet.authorProfileImageUrl} alt="" className="w-[50px] h-[50px] rounded-full" />
					<div className="flex flex-col overflow-hidden  gap-2 mt-4">
						<h3 className="font-semibold">
						{item.snippet.topLevelComment.snippet.authorDisplayName}<span className="font-[400] text-[14px] text-[#5a5a5a]">~ {moment(item.snippet.topLevelComment.snippet.publishedAt).fromNow()}</span>
						</h3>
						<div className='relative h-full'>
						<p className={`overflow-hidden h-[80px] ${item.id === targetId ? (commentOpen ? 'h-full' : ''):''}`}>{item.snippet.topLevelComment.snippet.textDisplay}</p>
							<span id={item.id} onClick={(e)=> handleClick(e)} className='font-semibold'>{(commentOpen ? (item.snippet.topLevelComment.snippet.textDisplay.length >= 250 ? '...less': ''):(item.snippet.topLevelComment.snippet.textDisplay.length >= 250 ? '...more': ''))}</span>
						</div>
